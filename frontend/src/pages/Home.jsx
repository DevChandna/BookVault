    ...
    const fetchFeaturedBooks = async () => {
      try {
        const response = await axios.get(
          `${import.meta.env.VITE_BACKEND_URL || 'http://localhost:8080'}/api/books`
        );
        const books = Array.isArray(response.data) ? response.data : [];
        setFeaturedBooks(books.slice(0, 6)); // Show first 6 books
      } catch (error) {
        console.error("Error fetching books:", error);
        setFeaturedBooks([]); // Ensure it's an array
      } finally {
        setLoading(false);
      }
    };
